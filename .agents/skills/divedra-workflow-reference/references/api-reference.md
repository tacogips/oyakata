# Divedra Workflow API Reference
...
Most library calls accept divedra load/session options:
...
```ts
import { createWorkflowExecutionClient } from "divedra";
...
```ts
import { rerunWorkflow, resumeWorkflow } from "divedra";
...
```ts
import { executeGraphqlRequest } from "divedra";
...
- Adds the divedra manager-session header when `managerSessionId` is provided.
...
Use when embedding divedra without HTTP:

```ts
import { createGraphqlSchema } from "divedra";
...
- The integration runs in the same process as divedra.
...
- You may later move execution behind `divedra serve`.
