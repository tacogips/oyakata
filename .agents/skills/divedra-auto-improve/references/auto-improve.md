# Divedra Auto Improve Reference
...
When nested supervision is enabled, divedra runs the configured supervisor workflow as a paired nested session and injects runtime variables such as supervision run id, target session id, and target workflow id.
