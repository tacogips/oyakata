---
name: divedra-workflow-organizer
...
metadata:
  short-description: Refactor divedra workflows
---

# Divedra Workflow Organizer

Use this skill to make existing divedra workflow bundles more DRY, maintainable, and effective without changing their intended behavior.

Use `divedra-workflow` for exact workflow schema rules while editing. Use `divedra-workflow-run` or `divedra-troubleshooting` when inspecting sessions, artifacts, or run failures.

## Organizing Workflow

1. Inventory workflows before editing:
   - List candidate bundles under the workflow root, usually `.divedra/workflows` or `examples`.
...
4. Improve input and output contracts from evidence:
   ...
   - Make step outputs match the next step's declared input needs; do not require command nodes to read divedra internal mailbox/session files unless that is the explicit product behavior being tested.
...
```bash
bun run src/main.ts workflow list --workflow-definition-dir .divedra/workflows
bun run src/main.ts workflow inspect <workflow-name> --workflow-definition-dir .divedra/workflows --output json
bun run src/main.ts workflow validate <workflow-name> --workflow-definition-dir .divedra/workflows
