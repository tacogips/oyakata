...
- If Step 5 returns `when.planning_only: true` and no revision flag is set, run Step 8 commit-message creation, then Step 9 command-node commit and push, before workflow output.
...
- After Step 7 accepts the implementation, run Step 8 commit-message creation, then Step 9 command-node commit and push, before workflow output.
