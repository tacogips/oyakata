...
- `cli workflow list`
  ...
  - In direct `--workflow-root` mode, list only workflows in that root and label their source scope as `direct`.
...
- `cli workflow status <name>`
  ...
  - In direct `--workflow-root` mode, resolve only within that root and label the source scope as `direct`.
