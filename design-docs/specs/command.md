...
- `cli workflow run <name>`
  ...
  - `--verbose` / `-v` prints local step-start progress to stderr as steps begin. This keeps `--output json` stdout parseable while giving operators immediate progress visibility for long local runs.
...
| `--verbose` / `-v`                                | boolean       | `false`                                             | For local workflow run/resume/rerun: print step-start progress to stderr without changing stdout output                                                                                      |
