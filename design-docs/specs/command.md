...
- `cli workflow run <name>`
  ...
  - `--auto-improve` remains accepted for explicit remediation policy. Auto-improve patching is not implied by ordinary deterministic lifecycle supervision, and `--no-auto-improve` remains the escape hatch for specialized unsupervised starts.
  ...
  - When `--endpoint` is used, the GraphQL execution transport must forward the same supervision policy surface as the local engine path; remote execution must not silently drop `--auto-improve`, `--no-auto-improve`, or nested-driver flags (`--nested-superviser` / `--nested-supervisor`). Because GraphQL defaults omitted `autoImprove` to supervised execution, remote `workflow run --no-auto-improve` must send `autoImprove: { enabled: false }`.
  - Local, endpoint-backed, GraphQL, and library start helpers must apply the same default-supervised decision. If no explicit `--auto-improve` policy is supplied and `--no-auto-improve` is absent, the effective start is supervisor-backed; if `--no-auto-improve` is present, the effective start is unsupervised and the opt-out must survive transport conversion.
...
| `--no-auto-improve`                               | boolean       | `false`                                             | For `workflow run`: opt out of default deterministic supervisor-backed lifecycle management and use legacy unsupervised start semantics; this flag is broader than disabling remediation.         |
